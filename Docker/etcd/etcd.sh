...
: ${CLUSTER_STATE:="new"}
  ...
  if [ ! -f /usr/local/bin/ETCD ]; then
	...
	if [ "$CLUSTER" ]; then
		...
		THIS_NAME="etcd"
		echo "etcd --data-dir=/data/etcd --name=${THIS_NAME} --advertise-client-urls=http://${THIS_IP}:${ETCD_PORT} --listen-client-urls=http://127.0.0.1:${ETCD_PORT},http://${THIS_IP}:${ETCD_PORT}" > /usr/local/bin/ETCD
	fi
	...
	if [ "$AUTO_TLS" ]; then
		...
		sed -i 's#$# --cert-file=/key/server.crt --key-file=/key/server.key#' /usr/local/bin/ETCD
		sed -i 's/http:/https:/g' /usr/local/bin/ETCD
		echo "https://$THIS_IP:$ETCD_PORT"
	else
